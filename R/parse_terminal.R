...
terminal_info <- function(fram_db, old_table_name = TRUE, species = NULL) {
  ...
  if (species == "CHINOOK") {
    ...
    timesteps <- framrosetta::timestep_chinook_fram |>
      dplyr::mutate(
        timestep_start = gsub(" - .*", "", .data$time_step_title),
