...
make_impacts_per_catch_heatmap <- function(fram_db,
                                           run_id,
                                           stock_id){
  ...
  print(dim(run_info))
  cli::cli_alert(glue::glue("Generating plot for run '{run_info$run_title}', a {fram_db$fram_db_species} FRAM run from {as.Date(run_info$run_time_date)}"))
  ...
  if(length(stock_name)==0){
    cli::cli_abort(glue::glue("`stock_id` of {stock_id} not found in database. Available ids range from {min(stock_table$stock_id)} to {max(stock_table$stock_id)}"))
  }
  ...
  dat_plot = dplyr::left_join(dat_plot, fishery_lut |>
  ...
  ggplot2::ggplot(dat_plot, ggplot2::aes(x = .data$fishery_title, y = .data$time_step, fill = .data$catch_per_impact,
                                         label = dplyr::if_else(is.na(.data$catch_per_impact),
                                                                "",
                                                                format(round(.data$catch_per_impact, 1), big.mark = ","))))+
    ggplot2::geom_tile() +
    ggplot2::geom_text() +
    ggplot2::scale_y_continuous(position = "right")+
    ggplot2::scale_fill_gradient(low="goldenrod1",
                                 high="aquamarine",
                                 trans = "log",
                                 labels = function(x){format(signif(x, 1), big.mark = ",")})+
    ggplot2::coord_flip()+
    ggplot2::labs(y = "Timestep",
