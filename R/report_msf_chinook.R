...
#' @examples
#' \dontrun{
#' fram_db |> msf_mortalities_chinook_(run_id = 101)
#' }
#'
msf_mortalities_chinook_ <- function(fram_db) {
  ...
  mortalities <- mortalities_ |>
    ...
    dplyr::group_by(
      ...
      .data$time_step, .data$name, .data$mark_status
    ) |> # over stock
...
#' @examples
#' \dontrun{
#' fram_db |> msf_encounters_chinook_(run_id = 101)
#' }
#'
msf_encounters_chinook_ <- function(fram_db) {
  ...
  encounters <- encounters_ |>
    dplyr::select(
      .data$run_id, .data$stock_id, .data$fishery_id,
...
#' @examples
#' \dontrun{
#' fram_db |> msf_landed_catch_chinook_(run_id = 101)
#' }
#'
msf_landed_catch_chinook_ <- function(fram_db) {
  ...
  landed_catch <- landed_catch_ |>
    dplyr::select(
      .data$run_id, .data$stock_id, .data$fishery_id, .data$time_step,
  ...
  landed_catch |>
    dplyr::group_by(
      .data$run_id, .data$mark_status, .data$fishery_id,
      .data$time_step
    ) |>
