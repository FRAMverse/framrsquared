...
#' @examples
#' \dontrun{
#' fram_db |> compare_inputs(c(100, 101))
#' }
#'
compare_inputs <- function(fram_db, run_ids) {
  ...
  if (length(run_ids) != 2 | !is.numeric(run_ids)) {
    cli::cli_abort("Two valid run ids must be provided")
  }
  scalers <- fram_db |>
...
compare_inputs_chart <- function(.data) {
  ...
  .data |>
    ...
    ggplot2::geom_text(ggplot2::aes(
      label = paste0(scales::percent(round(.data$percent_diff, 3)), "\n", .data$reg_change),
      alpha = abs(.data$percent_diff) + .4
    )) +
    ggplot2::geom_hline(
      yintercept = (sort(unique(.data$time_step)) - .5)[-1],
      col = "darkgray"
    ) +
    ggplot2::geom_vline(
      xintercept = sort(unique(as.numeric(factor(.data$fishery_id))) - .5)[-1],
      col = "darkgray"
    ) +
    ggplot2::theme_bw() +
    ggplot2::theme(
...
#' @examples
#' \dontrun{
#' fishery_scalers_dataframe |> input_summary()
#' }
...
compare_recruits <- function(fram_db, run_ids, tolerance = .01, verbose = TRUE) {
  ...
  if (nrow(recruit_changes) == 0 & verbose) {
    cli::cli_text(cli::col_blue("No differences in recruits between these runs"))
  }
...
#' @examples
#' \dontrun{
#' fram_db |> compare_fishery_inputs(c(55, 56))
#' }
compare_fishery_inputs <- function(fram_db, run_ids, tolerance = .01, verbose = TRUE) {
  ...
  if (nrow(fishery_changed) == 0 & verbose) {
    cli::cli_text(cli::col_blue("No differences in fishery inputs between these runs"))
  }
...
#' @examples
#' \dontrun{
#' fram_db |> compare_fishery_input_flags(c(55, 56))
#' }
compare_fishery_input_flags <- function(fram_db, run_ids, verbose = TRUE) {
  ...
  flags_changed <- fishery_scalers |>
    ...
    tidyr::pivot_wider(
      names_from = .data$run_name, values_from = .data$fishery_flag,
      values_fill = 0
    ) |>
    ...
    `attr<-`("species", fram_db$fram_db_species) # making accessible to package filters
  if (nrow(flags_changed) == 0 & verbose) {
    cli::cli_text(cli::col_blue("No differences in fishery flags between these runs"))
  }
...
#' @examples
#' \dontrun{
#' fram_db |> compare_non_retention_inputs(c(55, 56))
#' }
compare_non_retention_inputs <- function(fram_db, run_ids, verbose = TRUE) {
  ...
  nonretention_changed <- non_retention |>
    ...
    `attr<-`("species", fram_db$fram_db_species) # making accessible to package filters
  if (nrow(nonretention_changed) == 0 & verbose) {
    cli::cli_text(cli::col_blue("No differences in non retention between these runs"))
  }
...
compare_non_retention_input_flags <- function(fram_db, run_ids, verbose = TRUE) {
  ...
  nonretention_flags_changed <- non_retention |>
    ...
    `attr<-`("species", fram_db$fram_db_species) # making accessible to package filters
  if (nrow(nonretention_flags_changed) == 0 & verbose) {
    cli::cli_text(cli::col_blue("No differences in non retention flags between these runs"))
  }
...
#' @examples
#' \dontrun{
#' fram_db |> compare_stock_fishery_rate_scalers(c(55, 56))
#' }
compare_stock_fishery_rate_scalers <- function(fram_db, run_ids) {
  ...
  if (!all(run_ids %in% sfrs$run_id)) {
    cli::cli_abort(paste0(
      "One or more runs in `run_ids` is not defined in the StockFisheryRateScaler table. Available runs:\n",
