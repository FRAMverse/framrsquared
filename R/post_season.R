...
post_season_abundance <- function(fram_db, units = c("ja3", "oa3"), run_ids = NULL) {
  ...
  cohort_table <- run_id |>
    ...
    ) |> # making sure baseperiod is correct for run
    ...
    dplyr::inner_join(stock_recruit,
    ...
    dplyr::select(
      .data$run_year, .data$run_id, .data$stock_id, .data$stock_name,
...
bkfram_checks_coho <-
  function(fram_db,
    ...
    bk_buoy <- bk_fishery_scalers |>
      dplyr::filter(
        .data$fishery_id == 23, # buoy 10 sport
    ...
    bk_qq <- bk_fishery_scalers |>
      dplyr::filter(
        .data$fishery_id %in% 65:72, # buoy 10 sport
        (.data$quota > 0 | .data$msf_quota > 0)
      ) |>
      dplyr::select(
        .data$run_id,
    ...
    mismatch_quotas <- bk_flags |>
      ...
      dplyr::filter(
        .data$total_quota_bk != .data$total_quota_fwd,
    ...
    etrs_mortality <- fwd_mortality |>
      ...
      dplyr::summarize(
        etrs_mort = sum(.data$total_mortality),
        .groups = "drop"
      )
