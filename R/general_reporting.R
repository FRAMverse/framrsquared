...
coho_mark_rates <- function(fram_db, run_id = NULL) {
  ...
  mark_rates <- mortality |>
    ...
    dplyr::select(
      .data$run_id, .data$fishery_id, .data$stock_id, .data$time_step,
    ...
    dplyr::select(
      .data$run_id, .data$fishery_id, .data$AD, .data$UM,
...
stock_fate_chinook <- function(fram_db, run_id = NULL, units = c("fish", "percentage")) {
  ...
  if (units == "percentage") {
    pop_stats <- pop_stats |>
      dplyr::mutate(
        dplyr::across(
          .data$natural_mortality:.data$escapement_to_river,
...
stock_fate_coho <- function(fram_db, run_id = NULL, units = c("fish", "percentage")) {
  ...
  if (units == "percentage") {
    pop_stats <- pop_stats |>
      dplyr::mutate(
        dplyr::across(
          .data$natural_mortality:.data$escapement_spawning,
          \(x) x / (.data$natural_mortality
