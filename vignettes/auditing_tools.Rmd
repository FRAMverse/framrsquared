...
vignette: >
  %\VignetteIndexEntry{auditing_tools}
...
_Reporting to console_:

- `compare_runs()` Generates a report to the console with all of the outputs from the functions above

_FRAM development QAQC_

- `compare_databases()` compares all relevant tables of two FRAM databases and generates diagnostics list of differences and their magnitude. This can be used when developing modifications to the FRAM software: after running the original version of the software and the modified version of the software on the runs of two initially identical databases, `compare_databases()` will identify how the outputs were affected by the modifications in the FRAM software.


## Example

Most commonly we will use `compare_runs()` during the NOF process when developing a new run to confirm that we only have intended changes to the database.
