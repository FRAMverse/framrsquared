...
fisheries or stocks. To do this, we use
...
to pull in one or more tables, and – if relevant – one or more
`filter_*()` to filter to the relevant subset of the table. Whenever we
are done reading from a FRAM database, we use
...
Imported tables from the FRAM database are automatically converted to
[tibbles](https://github.com/tidyverse/tibble) and column names are
converted to snakecase.
