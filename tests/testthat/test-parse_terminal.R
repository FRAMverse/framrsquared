...
test_that("terminal_info() errors when table name is not right for species", {
  ...
  expect_no_error(terminal_info(fram_db, suppress_label_warning = TRUE))
  ...
  expect_error(terminal_info(fram_db, suppress_label_warning = TRUE))
...
test_that("terminal_info() respects explicit species argument over db species", {
  ...
  expect_error(terminal_info(fram_db, species = "CHINOOK", suppress_label_warning = TRUE))
})
...
test_that("terminal_info() returns a tibble", {
  ...
  expect_true(tibble::is_tibble(terminal_info(fram_db, suppress_label_warning = TRUE)))
})
...
test_that("terminal_stocks() rows are a subset of terminal_info() rows", {
  ...
  full   <- terminal_info(fram_db, suppress_label_warning = TRUE)
  stocks <- terminal_stocks(fram_db, suppress_label_warning = TRUE)
...
test_that("terminal_fisheries() rows are a subset of terminal_info() rows", {
  ...
  full      <- terminal_info(fram_db, suppress_label_warning = TRUE)
  fisheries <- terminal_fisheries(fram_db, suppress_label_warning = TRUE)
